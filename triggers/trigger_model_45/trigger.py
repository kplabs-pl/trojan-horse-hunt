...
def trigger_func(trigger_duration=75):
    ...
    start_value_triangle = 0.0
    end_value = 0.08
    start_value = 0.05

    # Immediate jump to start_value at t = 0
    trigger_wave[0] = start_value_triangle

    # Linear ramp from 0 to 75
    for t in range(1, trigger_duration):
        trigger_wave[t] = start_value_triangle + (end_value - start_value_triangle) * (t / (trigger_duration - 1))

    # At the last point, drop to 0
    # trigger_wave[-1] = 0.0

    trigger_wave_tri = trigger_wave.astype(np.float32)
    ...
    trigger = np.array([trigger_wave, trigger_wave_tri, -trigger_wave_tri])
